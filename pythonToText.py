...
#Discription: This program will take all the python files in your folder and folders in your folders and turn each one into a text file.
            #this is to help indexing and searching for specific code in a large folder of python files.

#WARNING:  This will overwrite any text files with the same name as the python files.  If you need an archive of the text files, make a folder with the name and date, and copy any text folders in the folders and subfolders into that folder.... or just copy the whole folder and copy it into another folder with the name and date.
...
#psuedo code:
"""
make a list of all the python files in the folder
read 1 file in that list of files
create and write the contents of the python file into the text file
...
#Note: funtion below will not work for a list of folders, only a single folder.  Basic code none the less for future use.

# def look_for_python_files_list(folder: str)->list:   
#     """Summery:
#     This funtion makes a list of all the python files in the folder.
    
#     Args:   
#         folder (str): The folder that the user wants to convert the python files into text files.
        
#     Returns:
#         list: A list of all the python files in the folder.
#     """
#     python_files = glob.glob(folder + '/*.py')
#     return python_files


def find_py_in_list(file_list: list) -> list:
    """Summary:
    This function sorts through the list of folders and makes a list of all the python files in one combined list.

    Args:
        file_list (list): A list of all the different folders to find python files.
    ...
    for element in file_list:
        element = str(element)
        python_files.extend(glob.glob(element + '/*.py'))
    return python_files
...
def py_to_txt_copy(file_name: str) -> str:
    ...
    text_file_name = file_name.replace('.py', '.txt')
...
def main():
    ...
    what_files_to_convert = find_py_in_list(folders)
    ...
    while what_files_to_convert != []:
        ...
        python_file_name = name_converted(what_files_to_convert)
        print(f'{python_file_name} has been converted')
